            ...
            <div class="col-lg-3 col-sm-12">
                {{ if isset .Params "images" }}
                    {{ with .Resources.GetMatch (index .Params.Images 0) }}
                        {{ $image := .Resize "200x webp" }}
                        <img class="img-fluid rounded w-100" src="{{ $image.Permalink }}" loading="lazy" {{/* width="{{ $image.Width }}" height="{{ $image.Height }}" */}} alt="featured image {{.Title }}"/>
                    {{ end }}
                {{ end }}
            </div>
                        ...
                        <div class="card-subtitle text-muted">
                            ...
                            {{ if isset .Params "categories" }}
                                ...
                                {{ range .Params.categories }}
                                    <a class="text-decoration-none text-muted ms-2" href="/categories/{{ . | urlize}}">{{ . }}</a>
                                {{end}}
                            {{ end }}
                            ...
                            {{ if isset .Params "tags" }}
                                ...
                                {{ range .Params.tags }}
                                    <a class="text-decoration-none text-muted ms-2" href="/tags/{{ . | urlize}}">{{ . }}</a>
                                {{ end }}
                            {{ end }}
                        </div>
